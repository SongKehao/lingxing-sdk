...
(function () {
  ...
  var mediumZoom = function mediumZoom(selector) {
    ...
    var _handleClick = function _handleClick(event) {
      ...
      /*** 请注意：由于地址指针发生变化，所以不能使用indexOf方式进行判断！ ***/ 
      if (!images.filter(item => item.src == target.src).length) {
        return;
      }
    ...
    var open = function open() {
      ...
      return new Promise(function (resolve) {
        /*** 请注意：由于地址指针发生变化，所以不能使用indexOf方式进行判断！ ***/ 
        if (target && !images.filter(item => item.src == target.src).length) {
          resolve(zoom);
          return;
        }
    ...
    /**
     * 存在问题：由于images定义在内部，而每次切换路由时都会导致images在堆中重新分配，所以需要将该变量保存至外部，只保存一次
     * 
     * */ 
    ...
    /**
     * 存在问题：每次切换路由时，以下事件都会重新绑定一次回调函数，有以下两种方式解决
     * 1. 每次路由销毁时，解绑以下事件回调
     * 2. 简单添加一个变量，让事件只绑定一次
     * */ 
    if (!isPluginLoad) {
