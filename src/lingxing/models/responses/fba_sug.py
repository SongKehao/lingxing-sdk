...
class FbasugAsinGetdailysalesinfofeatureSugDateLine(LingXingModel):
    ...
    type: Optional[str] = Field(
        None,
        description="日期类型： oversea_arrive_date 海外仓到货日 with_safe_day_date 安全天数日 with_days_frequency_date 补货频率日 local_arrive_date 本地到货日 air_arrive_date 空运到货日 sea_arrive_date 海运到货日",
    )
...
class FbasugAsinGetdailysalesinfofeatureResponse(LingXingModel):
    ...
    total: Optional[int] = Field(None, description="总数")
    list: Optional[List[FbasugAsinGetdailysalesinfofeatureList]] = Field(
        None, description="每日数据信息【按天返回360天的数据】"
    )
...
class FbasugAsinGetinfoResponse(LingXingModel):
    ...
    quantity_sug_local_to_oversea_air: Optional[float] = Field(
        None, description="已弃用（建议本地发海外仓量（空运）（海外仓模式））"
    )
    quantity_sug_local_to_oversea_sea: Optional[float] = Field(
        None, description="已弃用（建议本地发海外仓量（海运）（海外仓模式））"
    )
...
class FbasugAsinGetsourcelistSourceList(LingXingModel):
    ...
    type: Optional[dict] = Field(
        None,
        description="数据类型： 1 FBA可售 2 FBA在途 3 本地可用 4 待检量 5 待交付 6 采购计划 8 海外仓可用 9 海外仓在途",
    )
...
class FbasugMskuGetdailysalesinfofeatureResponse(LingXingModel):
    ...
    total: Optional[int] = Field(None, description="总数")
    list: Optional[List[FbasugMskuGetdailysalesinfofeatureList]] = Field(
        None, description="每日数据信息【按天返回360天的数据】"
    )
...
class RestockingAnalysisGetsummarylistBasicInfo(LingXingModel):
    ...
    node_type: Optional[int] = Field(
        None, description="节点类型： 1 共享库存父行 2 共享库存子行 3 非共享库存 4 ASIN+国家汇总行"
    )
...
class RestockingAnalysisGetsummarylistResponse(LingXingModel):
    ...
    amazon_quantity_info: Optional[List[RestockingAnalysisGetsummarylistAmazonQuantityInfo]] = Field(
        None, description="亚马逊数量"
    )
    scm_quantity_info: Optional[List[RestockingAnalysisGetsummarylistScmQuantityInfo]] = Field(
        None, description="供应链数量"
    )
