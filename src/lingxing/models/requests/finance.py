...
class FinanceFeemanagementlistRequest(LingXingModel):
    ...
    apportion_status: Optional[list] = None  # 分摊状态【未设置新利润报表启用月使用该入参】： 1 未分摊 2 已分摊-新 3 已分摊-旧 4 已分摊
    status_merge: Optional[int] = None  # 分摊状态【已设置新利润报表启用月使用该入参】： 1 未分摊 2 已分摊
    search_field: Optional[str] = None  # 搜索类型： number 单据编号 msku MSKU asin ASIN create_name 创建人 remark_order 单据备注 remark_item 明细备注
...
class FinanceBdmskuRequest(LingXingModel):
    ...
    orderStatus: Optional[str] = None  # 交易状态 Deferred 已推迟 Disbursed 已发放【默认】 DisbursedAndPreSettled 已发放（含预结算） All 全部（不包含已发放预结算数据）
...
class FinanceBdasinRequest(LingXingModel):
    ...
    orderStatus: Optional[str] = None  # 交易状态 Deferred 已推迟 Disbursed 已发放【默认】 DisbursedAndPreSettled 已发放（含预结算） All 全部
...
class FinanceProfitreportordertranscationlistRequest(LingXingModel):
    ...
    mids: Optional[list] = None  # 站点id
    sids: Optional[list] = None  # 店铺id ，对应查询亚马逊店铺列表接口对应字段【sid】
    searchDateField: Optional[str] = None  # 时间筛选类型: posted_date_locale 结算时间【默认】 fund_transfer_datetime_locale 转账时间 shipment_datetime_locale 发货时间
    startDate: str  # 开始时间
    endDate: str  # 结束时间
    ...
    currencyCode: Optional[str] = None  # 货币种类: 原币种【默认】 CNY USD EUR JPY AUD CAD MXN GBP INR AED SGD SAR BRL SEK PLN TRY HKD
    searchField: Optional[str] = None  # 查询索引字段: order_id 订单号【默认】 seller_sku MSKU asin ASIN parent_asin 父ASIN local_sku SKU local_name 品名gmt_
    ...
    orderStatus: Optional[str] = None  # 交易状态 Deferred 已推迟（结束时间必须要今天才能获取已推迟数据） Disbursed 已发放【默认】 DisbursedAndPreSettled 已发放（含`预结`算） All 全部
...
class FinanceBdorderRequest(LingXingModel):
    ...
    start_date: str  # 开始时间
    end_date: str  # 结束时间
    search_field: Optional[str] = None  # 搜索值类型： 亚马逊订单号：order_id MSKU：seller_sku Asin：asin 父asin：parent_asin SKU：local_sku 品名：local_name
    ...
    fund_transfer_status: Optional[list] = None  # 转账状态： 已转帐 Succeeded 转帐中 Processing 失败 Failed 未知 Unknown
    event_source: Optional[list] = None  # 来源： SellerDealPayment ServiceFee Adjustment Refund SellerReviewEnrollmentPayment RemovalShipmentAdju
...
class FinanceSettlementtransactionlistRequest(LingXingModel):
    ...
    sids: Optional[list] = None  # 店铺id ，对应查询亚马逊店铺列表接口对应字段【sid】
    startDate: Optional[str] = None  # 开始时间【结算时间】，双闭区间，查询时间间隔不得超过7天，格式：Y-m-d 【无搜索值时，结算时间、修改时间二选一必填】
    endDate: Optional[str] = None  # 结束时间【结算时间】，双闭区间，查询时间间隔不得超过7天，格式：Y-m-d 【无搜索值时，结算时间、修改时间二选一必填】
    eventType: Optional[str] = None  # 来源，多个英文用逗号隔开，枚举见附加说明
    type: Optional[str] = None  # 交易类型
    searchField: Optional[str] = None  # 搜索字段： id 结算编号 amazon_order_id 订单编号 primary_id 主键编号【对应本接口返回id值】 settlement_id 账单编号【此项下，结算时间或更新时间必填】
    searchValue: Optional[list] = None  # 搜索值
    gmtModifiedStart: Optional[str] = None  # 修改开始时间（北京时间），格式：Y-m-d H:i:s 【无搜索值时，结算时间、修改时间二选一必填】
    gmtModifiedEnd: Optional[str] = None  # 修改结束时间（北京时间），格式：Y-m-d H:i:s 【无搜索值时，结算时间、修改时间二选一必填】
...
class FinanceCenterodsdetailqueryRequest(LingXingModel):
    ...
    eventTypes: Optional[list] = None  # 事件类型，支持传多值： 01 Shipments 02 CustomerReturns 03 WhseTransfers 04 Receipts 05 VendorReturns 06 Adjustm
...
class FinanceInvoiceListRequest(LingXingModel):
    ...
    ads_type: Optional[list] = None  # 广告类型： SPONSORED PRODUCTS SPONSORED DISPLAY SPONSORED BRANDS SPONSORED BRANDS VIDEO
    invoice_start_time: str  # 开始时间【发票开具时间】
    invoice_end_time: str  # 结束时间【发票开具时间】
    search_type: Optional[str] = None  # 搜索类型： ads_campaign【对应页面广告活动】 invoice_id【对应发票编号】 msku asin
    search_value: Optional[str] = None  # 搜索值
...
class FinanceRequestFundsOrderListRequest(LingXingModel):
    ...
    search_field_time: Optional[str] = None  # 搜索时间类型： apply_time 申请时间 real_pay_time 实际付款时间 prepay_time 预计付款时间
...
class FinanceQueryReceiptFundsListRequest(LingXingModel):
    ...
    offset: Optional[int] = None  # 分页偏移量，默认0
    searchField: Optional[str] = None  # 搜索字段，必填，枚举值：supplier_name-供应商, order_sn-收款单号, purchase_order_sn-采购单号, purchase_return_order_sn-退货单号,
    ...
    status: Optional[list] = None  # 状态筛选，String数组，枚举值：1-待收款, 2-已完成, 3-已作废, 121-待审批, 122-已驳回, 124-已作废
...
class FinanceRequestfundspoollogisticslistRequest(LingXingModel):
    ...
    offset: Optional[int] = None  # 分页偏移量，默认0
    length: Optional[int] = None  # 分页长度，默认20，上限200
    search_field_time: Optional[str] = None  # 时间搜索类型： create_time  费用录入时间   delivery_create_time  发货单创建时间  shipment_time  发货时间  close_time  付清时间
    start_time: Optional[str] = None  # 开始时间【时间间隔最长不得超过90天】，闭区间，格式：Y-m-d
    end_time: Optional[str] = None  # 结束时间【时间间隔最长不得超过90天】，闭区间，格式：Y-m-d
...
class FinanceRequestfundspoolotherfeelistRequest(LingXingModel):
    ...
    searchField: Optional[str] = None  # 搜索字段，枚举值：order_sn-采购单号, create_username-采购员，配合searchValue使用
    searchFieldTime: Optional[str] = None  # 时间维度，枚举值：create_time-创建时间, close_time-付清时间，默认create_time
...
class FinanceReceivablereportlistRequest(LingXingModel):
    ...
    sortField: Optional[str] = None  # 排序字段： beginningBalanceCurrencyAmount 期初余额 incomeAmount 收入 refundAmount 退款 spendAmount 支出 other 其他
...
class FinanceOrderProfitListMSKURequest(LingXingModel):
    ...
    searchValue: Optional[list] = None  # 搜索的值
    currencyCode: Optional[str] = None  # 币种code【默认原币种】, 可选值：原币种,CNY,USD,EUR,JPY,AUD,CAD,MXN,GBP,INR,AED,SGD,SAR,BRL,SEK,PLN,TRY,HKD
...
class FinanceLazadasettlementlistRequest(LingXingModel):
    ...
    endDate: Optional[str] = None  # 结束时间，支持 `yyyy-MM-dd` 或 `yyyy-MM-dd HH:mm:ss` 格式，仅日期时自动补充 `23:59:59`
    ...
    searchType: Optional[float] = None  # 搜索类型： `1` 平台单号 `2` 平台子单号 `3` 交易编号 `4` 支付参考编号 `5` 参考单号
    ...
    startDate: Optional[str] = None  # 开始时间，支持 `yyyy-MM-dd` 或 `yyyy-MM-dd HH:mm:ss` 格式，仅日期时自动补充 `00:00:00`
...
class FinanceShopeeadjustmentlistRequest(LingXingModel):
    ...
    adjTypes: Optional[list] = None  # 调整类型数组（下拉多选）： `Refund Amount` `Marketing Fee` `Warehouse Fee` `Other` 其他类型
    compareLogic: Optional[str] = None  # 字段比较条件逻辑关系： `AND` 所有条件都满足 `OR` 满足任一条件 默认 `AND`
...
class FinanceShopeeincomelistRequest(LingXingModel):
    ...
    searchSingleValue1: Optional[str] = None  # 商品搜索值（单个，支持模糊匹配，多个值用逗号分隔）
    searchType: Optional[float] = None  # 搜索类型： `1` 平台单号
    searchType1: Optional[float] = None  # 商品搜索类型： `11` MSKU ID `12` MSKU `13` MSKU 名称 `14` 商品ID `15` 全球商品货号 `16` 商品名称
    sids: Optional[list] = None  # 店铺 ID 数组
    sites: Optional[list] = None  # 站点数组
    sortField: Optional[str] = None  # 排序字段
    sortType: Optional[str] = None  # 排序方向： `1` 升序/ASC `0` 降序/DESC 默认 `0`
