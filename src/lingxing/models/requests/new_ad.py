...
class NewadReportSptargethourdataRequest(LingXingModel):
    ...
    campaign_id: float  # 广告活动id
    agg_dimension: (
        str  # 聚合维度: target  投放维度 both_ad_target  广告+投放维度   both_target_placement 投放+广告位placement维度
    )
...
class NewadBasedataNewadsbdivideasinreportsRequest(LingXingModel):
    ...
    next_token: Optional[str] = (
        None  # 分页游标，上次分页结果中的next_token (第一次分页无需填写，当next_token和offset同时存在时以next_token为主
    )
...
class NewadBasedataPortfoliosRequest(LingXingModel):
    ...
    length: Optional[int] = None  # 分页长度，默认15
    next_token: Optional[str] = (
        None  # 分页游标，上次分页结果中的next_token (第一次分页无需填写，当next_token 和 offset同时存在时以next_token为主
    )
...
class NewadBasedataSbtargetingRequest(LingXingModel):
    ...
    targeting_type: (
        str  # 投放类型： keyword 返回关键词数据 producttarget 返回商品定位数据 ALL：同时返回关键词和商品定位数据
    )
    offset: Optional[int] = None  # 分页偏移量，默认0
