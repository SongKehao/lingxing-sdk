...
class MultiPlatformAdvertisementTiktokAdGroupList12Request(LingXingModel):
    ...
    objectiveType: Optional[list] = (
        None  # 推广目标列表，String数组，枚举值：REACH-覆盖人数, TRAFFIC-访问量, VIDEO_VIEWS-视频播放量, LEAD_GENERATION-线索收集, ENGAGEMENT-社区互
    )
    ...
    searchType: Optional[str] = (
        None  # 搜索字段，当字段searchValue有值时，该字段也必须有值，且根据报告类型填写对应的值(advertiser_name-广告账号,ad_group_name-广告组,campaign_name推广
    )
    searchValue: Optional[list] = None  # 搜索值，String数组
    serviceStatus: Optional[list] = None  # 服务状态列表，String数组
    status: Optional[list] = (
        None  # 状态列表，String数组，枚举值：STATUS_ENABLE-已启用, SYSTEM_STATUS_IN_REVIEW-审核中, SYSTEM_STATUS_NOT_PASS-未通过, STATUS
    )
...
class MultiPlatformAdvertisementTiktokAdList13Request(LingXingModel):
    ...
    ownerBcIds: Optional[list] = None  # 广告主BusinessId列表，Long数组
    searchType: Optional[str] = (
        None  # 搜索字段，枚举值：advertiser_name-广告账号, ad_group_name-广告组, campaign_name-推广系列, ad_name-广告
    )
    searchValue: Optional[list] = None  # 搜索值，String数组
    serviceStatus: Optional[list] = None  # 服务状态列表，String数组
    status: Optional[list] = (
...
class MultiPlatformAdvertisementTiktokAdvertiserList2Request(LingXingModel):
    ...
    ownerBcIds: Optional[list] = None  # 广告主BusinessId列表，Long数组
    searchType: Optional[str] = (
        None  # 搜索字段，当字段searchValue有值时该字段也必须有值，枚举值：advertiser_name-广告账号, ad_group_name-广告组, campaign_name-推广系列, ad_n
    )
...
class MultiPlatformAdvertisementTiktokCampaignList14Request(LingXingModel):
    ...
    ownerBcIds: Optional[list] = None  # 广告主BusinessId列表，Long数组
    searchType: Optional[str] = (
        None  # 搜索字段，枚举值：advertiser_name-广告账号, ad_group_name-广告组, campaign_name-推广系列, ad_name-广告。当字段searchValue有值时，该
    )
    searchValue: Optional[list] = None  # 搜索值，String数组
    serviceStatus: Optional[list] = None  # 服务状态列表，String数组
    status: Optional[list] = (
...
class MultiPlatformAdvertisementTiktokCommonAdvertiserList4Request(LingXingModel):
    ...
    internalStatus: Optional[str] = (
        None  # 内部状态，枚举值：ENABLE-启用, DISABLE-禁用, DELETE-删除。用于过滤授权信息表中的状态，不传则返回所有状态的广告账号
    )
...
class MultiPlatformAdvertisementTiktokGmvAdvertiserReportList5Request(LingXingModel):
    ...
    gmvMaxPromotionTypeCodes: Optional[list] = (
        None  # GMV Max类型编码列表，String数组，枚举值：PRODUCT-商品GMV, LIVE-直播GMV
    )
    orderField: Optional[str] = None  # 排序字段名称，如：cost, orders, roi
    orderType: Optional[str] = None  # 排序方式，枚举值：ASC-升序, DESC-降序
    ownerBcIds: Optional[list] = None  # 广告主账号ID列表，Long数组，业务负责人的BC ID列表
    status: Optional[list] = (
        None  # 广告账号状态编码列表，String数组，枚举值：STATUS_ENABLE-已启用, SYSTEM_STATUS_IN_REVIEW-审核中, SYSTEM_STATUS_NOT_PASS-未通过,
    )
...
class MultiPlatformAdvertisementTiktokGmvCampaignReportList6Request(LingXingModel):
    ...
    status: Optional[list] = (
        None  # 推广系列操作状态编码列表，String数组，枚举值：ENABLE-已开启, DISABLE-已暂停, DELETE-已删除
    )
    storeIds: Optional[list] = None  # 店铺ID列表，Long数组，用于筛选特定店铺的数据
...
class MultiPlatformAdvertisementWalmartAdGroupSvList1Request(LingXingModel):
    ...
    campaignType: Optional[list] = (
        None  # 广告活动类型列表，String数组，枚举值：sponsoredProducts-manual(SP手动), sponsoredProducts-auto(SP自动), sba(SB品牌广告), vid
    )
    ...
    orderField: Optional[str] = (
        None  # 排序字段，支持对查询结果中的任意字段进行排序（驼峰命名）。包括但不限于: 基础指标(numAdsShown/numAdsClicks/adSpend)、销售指标(attributedSales/att
    )
...
class MultiPlatformAdvertisementWalmartCampaignSpList3Request(LingXingModel):
    ...
    orderField: Optional[str] = (
        None  # 排序字段，支持对查询结果中的任意字段进行排序（驼峰命名）。包括但不限于：基础指标(numAdsShown/numAdsClicks/adSpend)、销售指标(attributedSales/attr
    )
    orderType: Optional[str] = None  # 排序类型，枚举值：ASC-升序, DESC-降序。不传时默认ASC
    searchText: Optional[str] = None  # 搜索文本，模糊搜索广告活动名称
    status: Optional[list] = (
        None  # 广告活动状态列表，String数组，枚举值：enabled-启用, paused-暂停, scheduled-已安排, rescheduled-重新安排, live-运行中, proposal-提议,
    )
...
class MultiPlatformAdvertisementWalmartGroupSpList9Request(LingXingModel):
    ...
    campaignType: Optional[list] = (
        None  # 广告活动类型列表，必填，String数组，枚举值：sponsoredProducts-manual-SP手动, sponsoredProducts-auto-SP自动, sba-SB品牌广告, vid
    )
...
class MultiPlatformAdvertisementWalmartReportadgroupsblist15Request(LingXingModel):
    ...
    campaignType: Optional[list] = (
        None  # 广告活动类型列表，String数组，必填，枚举值：sponsoredProducts-manual(SP手动), sponsoredProducts-auto(SP自动), sba(SB品牌广告),
    )
    endDate: Optional[str] = None  # 结束日期，必填，格式：yyyy-MM-dd，且 startDate 和 endDate 间隔不能超过31天
    startDate: Optional[str] = None  # 开始日期，必填，格式：yyyy-MM-dd，且 startDate 和 endDate 间隔不能超过31天
    campaignIds: Optional[list] = None  # 广告活动ID列表，Long数组，按广告活动ID筛选广告组
    day: Optional[int] = None  # 归因天数，数据归因天数，枚举值：3, 14, 30，默认14天
    orderField: Optional[str] = (
        None  # 排序字段，支持对查询结果中的任意字段进行排序（驼峰命名）。包括：基础指标(numAdsShown/numAdsClicks/adSpend)、销售指标(attributedSales/attribut
    )
    orderType: Optional[str] = None  # 排序类型，枚举值：ASC-升序, DESC-降序，不传时默认ASC
    pageNum: Optional[int] = None  # 页码，分页时的页码，从1开始
...
class MultiPlatformAdvertisementWalmartReportpagetypesblist24Request(LingXingModel):
    ...
    pageNum: Optional[int] = None  # 页码，分页时的页码，从1开始
    pageSize: Optional[int] = None  # 每页大小，分页时每页显示的记录数，最大200
    pageType: Optional[list] = (
        None  # 页面类型列表，String数组，枚举值：browse-浏览, item-商品, search-搜索, topic-主题, category-分类, homepage-首页, other-其他
    )
    paging: Optional[bool] = None  # 是否分页，默认为true
    searchText: Optional[str] = None  # 搜索文本，模糊搜索广告活动名称（campaign_name）
    searchType: Optional[str] = None  # 搜索类型，目前不用传
...
class MultiPlatformAdvertisementWalmartReportsearchtrendslist28Request(LingXingModel):
    ...
    itemBrand: Optional[dict] = (
        None  # 商品品牌(在item_brand_1/2/3中搜索)，模糊搜索请使用String类型，精确搜索请使用数组类型
    )
    ...
    orderField: Optional[str] = (
        None  # 排序字段(驼峰格式)，枚举值：searchKeyword-搜索关键词, keywordRank-关键词排名, totalPctClickShare-前3商品点击占比总和, totalPctConvSh
    )
...
class MultiPlatformAdvertisementShopeecampaignreportlistRequestFilterItem(LingXingModel):
    ...
    statusCodes: Optional[list] = (
        None  # 广告活动状态列表（复选，为空时为全部）： `ongoing` 进行中 `scheduled` 已计划 `ended` 已结束 `paused` 已暂停 `deleted` 已删除 `closed` 已
    )
    placementCategories: Optional[list] = (
        None  # 广告位类别列表（复选，为空时为全部）： `search` 搜索 `discovery` 展示 `all` 全部
    )
...
class MultiPlatformAdvertisementLazadaAudienceReportListRequestFilterItem(LingXingModel):
    ...
    audienceGroups: Optional[list] = (
        None  # 受众分组列表1=过去15天访问, 2=浏览相似商品, 3=店铺触达受众, 4=店铺兴趣受众, 5=DMP受众, 6=性别, 7=年龄
    )
...
class MultiPlatformV2ProfitreportmskuRequest(LingXingModel):
    ...
    platformCodeS: Optional[list] = (
        None  # 平台id： 10002 Shopify 10003 eBay 10005 AliExpress 10006 Shopee  10008 walmart 10011 Tiktok 10021 Shein
    )
...
class MultiPlatformV2ProfitreportorderRequest(LingXingModel):
    ...
    platformCodeS: Optional[list] = (
        None  # 平台id： 10002 Shopify 10003 eBay 10005 AliExpress 10006 Shopee 10007 Lazada  10008 walmart 10011 Tikto
    )
    mids: Optional[str] = None  # 国家id，多个使用英文逗号分隔
    sids: Optional[str] = None  # 店铺id，多个使用英文逗号分隔 ，对应查询多平台店铺信息接口对应字段【store_id】
    ...
    searchField: Optional[str] = (
        None  # 搜索值类型：msku MSKU，local_sku SKU，product_name，品名，platform_order_no 平台单号
    )
    searchValue: Optional[str] = None  # 搜索值
...
class MultiPlatformV2StoreInfoV2Request(LingXingModel):
    ...
    platform_code: Optional[list] = (
        None  # 平台code： 10001 AMAZON 10002 Shopify 10003 eBay 10004 Wish 10005 AliExpress 10006 Shopee 10007 Lazada
    )
...
class MultiPlatformV2MultiPlatOrderV2Request(LingXingModel):
    ...
    date_type: Optional[str] = (
        None  # 时间类型： 更新时间 update_time  订购时间 global_purchase_time  发货时间 global_delivery_time 付款时间 global_payment_tim
    )
    start_time: Optional[int] = (
        None  # 开始时间，时间戳格式【单位：秒】，双开区间 **当且仅当传入平台单号或平台单名称查询时可不必传，查询时间跨度不能超过31天**
    )
    end_time: Optional[int] = (
        None  # 结束时间，时间戳格式【单位：秒】，双开区间 **当且仅当传入平台单号或平台单名称查询时可不必传，查询时间跨度不能超过31天**
    )
    store_id: Optional[list] = None  # 店铺id，取值等同于查询多平台店铺信息返回结果的store_id
    platform_code: Optional[list] = (
        None  # 平台code： 10001 AMAZON 10002 Shopify 10003 eBay 10004 Wish 10005 AliExpress 10006 Shopee 10007 Lazada
    )
    platform_order_nos: Optional[list] = (
        None  # 平台单号列表 ，元素不超过200个 **以下平台不可用，需要用platform_order_names查询：** **10003-ebay 10014-newegg 10020-coupang 100
    )
    platform_order_names: Optional[list] = (
        None  # 特定平台单号列表 ，元素不超过200个 **10003-ebay 10014-newegg 10020-coupang 10002-shopify 10012-美客多 10016-shopline，使
    )
    order_status: Optional[int] = (
        None  # 订单状态： 1 同步中 2 已同步 3 待付款 4 待审核 5 待发货 6 已发货 7 已取消/不发货 8 不显示 9 平台发货
    )
    platform_shipping_status: Optional[list] = (
        None  # 平台单发货状态 **Shopify**状态枚举值: fulfilled：已发货，并且全部发货 null：未发货 partial：部分发货 restocked：已退货
    )
    platform_payment_status: Optional[list] = (
        None  # 平台单支付状态 **Shopify**状态枚举值: pending：待支付 authorized：买家信用卡支付，并且已经确认授权，但是卖家并未收款 partially_paid：已完成部分款项支付
    )
...
class MultiPlatformV2PairListV2Request(LingXingModel):
    ...
    use_cursor: Optional[bool] = (
        None  # 分页游标，默认为fasle，如配对数据多时，强烈建议您使用分页游标的方式分页，可加快接口响应速度
    )
...
class MultiPlatformV2CreateOrdersV2RequestOrdersItem(LingXingModel):
    ...
    sender_tax_type: Optional[int] = (
        None  # "税号类型: 1=>IOSS编号 2=>VAT税号 3=>CPF税号 4=>EORI号码 5=>收件人税号 7=>VOEC税号"
    )
...
class MultiPlatformV2QueryShippingListV2Request(LingXingModel):
    ...
    timeField: Optional[int] = (
        None  # 时间维度 1 创建时间 2 发货时间 3 开船时间 4 预计到港时间 5 实际妥投时间 6 实际发货时间
    )
...
class MultiPlatformV2QueryWFSCargoPageRequest(LingXingModel):
    ...
    store_id: Optional[list] = None  # 店铺id
    cargo_status_list: Optional[list] = (
        None  # 货件平台状态： 0 PENDING_SHIPMENT_DETAILS 1 AWAITING_DELIVERY 2 RECEIVING_IN_PROGRESS 3 CLOSED 4 CANCELLED
    )
...
class MultiPlatformV2TemuCargoRequest(LingXingModel):
    ...
    statusList: (
        List  # 待发货：0 ；待收货：1 ；已收货：2 ；已入库：3 ；已退货：4 ；已取消：5 ；部分收货：6 ;待申报（本地状态）7
    )
...
class MultiPlatformV2SheinListRequest(LingXingModel):
    ...
    searchField: Optional[str] = (
        None  # 搜索字段 1、标题 2、品名 3、SPU货号 4、SKC货号 5、平台SPU 6、平台SKC 7、MSKU ID 8、SKU 9、MSKU
    )
...
class MultiPlatformV2TikTokListRequest(LingXingModel):
    ...
    platformStatus: Optional[list] = (
        None  # 状态 DRAFT PENDING FAILED ACTIVATE SELLER_DEACTIVATED PLATFORM_DEACTIVATED FREEZE DELETED
    )
    storeIds: Optional[list] = None  # 店铺id列表
...
class MultiPlatformV2WalmartlistRequest(LingXingModel):
    ...
    store_ids: Optional[list] = None  # 店铺id
    status: Optional[list] = (
        None  # 状态： 0 PUBLISHED 1 READY TO PUBLISH 2 IN PROGRESS 3 UNPUBLISHED 4 STAGE 5 SYSTEM PROBLEM
    )
...
class MultiPlatformV2NewplatformorderlistRequest(LingXingModel):
    ...
    platformCodeList: Optional[list] = (
        None  # 平台CODE，目前仅支持 TikTok、TEMU 半托管、Line Shopping、Lazada、Shopee、Shopify、Walmart、Wayfair 平台
    )
    ...
    sortField: Optional[str] = (
        None  # 排序字段列表字段支持：purchaseTime，paymentTime，platformOrderModifiedTime,deliveryTime
    )
...
class MultiPlatformV2TemustockorderquerypageRequest(LingXingModel):
    ...
    bizStatusList: Optional[list] = (
        None  # 单据状态列表。 0 待接单 1 待发货 2 已送货 3 已收货 5 质检全部退回 6 已验收 7 已入库 8 已作废 9 已超时
    )
    ...
    searchType: Optional[float] = (
        None  # 搜索类型。 0 备货单号 1 货件号 2 SKC 3 MSKU 4 SPU 5 SKU 6 品名 7 备注 8 MSKU_CODE。当传入 `searchValueList` 时，`searchTyp
    )
    fuzzySearchValue: Optional[str] = None  # 模糊搜索值，多个值时以下游要求的换行符拼接
    searchValueList: Optional[list] = (
        None  # 批量搜索值列表。传入后系统会自动以换行符拼接。当传入 `searchValueList` 时，`searchType` 必传。
    )
