...
class ServiceReviewv2Request(RequestBase):
    ...
    # 店铺id，多个用逗号分隔 ，对应查询亚马逊店铺列表接口对应字段【sid】
    mids: Optional[str] = None
    # 站点id，多个用逗号分隔
    principal_uids: Optional[str] = None
    # lisitng负责人，多个用逗号分隔
    search_field: Optional[str] = None
    # 搜索字段: asin ASIN parent_asin 父ASIN remark 备注 amazon_order_id 订单号 author 买家信息 review_id  Review ID buy
    search_value: Optional[str] = None
    # 搜索值
    date_field: str
    # 时间搜索类型: review_time 评价时间 create_time 创建时间 last_update_time 更新时间
    start_date: str
    # 开始时间，格式：Y-m-d
    end_date: str
    # 结束时间，格式：Y-m-d
    ...
    # 星级，多个用逗号分隔
    review_modified_status: Optional[str] = None
    # 内容，多个用逗号分隔: -1 已删除 0 未标识 1 已变更
    mark: Optional[str] = None
    # 标识，多个用逗号分隔: is_vp is_er is_topc is_topr is_vine
    cs_principal_uids: Optional[str] = None
    # 处理人，多个用逗号分隔
    offset: Optional[int] = None
    # 分页偏移量，默认0
    length: Optional[int] = None
    # 分页长度，默认20，上限200
    cids: Optional[str] = None
    # 分类id，多个用逗号分隔
    global_tag_ids: Optional[str] = None
    # 标签id，多个用逗号分隔
...
class ServiceAftersalesworkorderlistRequest(RequestBase):
    ...
    start_time: str
    # 开始时间，闭区间，格式：Y-m-d H:i:s
    end_time: str
    # 结束时间，闭区间，格式：Y-m-d H:i:s
    offset: int
    # 分页偏移量
    length: int
    # 分页长度，上限500
...
class ServiceStoreperformancelistRequest(RequestBase):
    ...
    length: Optional[int] = None
    # 分页长度，默认20，上限200
    search_field_time: Optional[str] = None
    # 搜索时间类型： pull_date 报表获取时间  update_date 更新时间
...
class ServiceVoiceofbuyerlistRequest(RequestBase):
    ...
    # 配送方式： FBA  FBA MFN  FBM
    sids: Optional[list] = None
    # 店铺id ，对应查询亚马逊店铺列表接口对应字段【sid】
    pxc_health: Optional[list] = None
    # 满意度状况： -1  反馈不足 0  极差 1  不合格 2  一般 3  良好 4  极好
    search_field: Optional[str] = None
    # 搜索类型： asin  ASIN msku   MSKU sku   SKU
...
class ServiceCustomerservicermamanagelistRequest(RequestBase):
    ...
    # 店铺id，支持多选，数组
    searchTimeFiled: str
    # 搜索时间类型：1创建时间 2.操作时间   createTime operationTime
    startTime: str
    # 创建或完成时间（开始），精确到年月日，无默认
    endTime: str
    # 创建或完成时间（开始），精确到年月日，无默认
    searchValue: List
    # 搜索值，msku和asin支持多个搜索，数组
    searchField: str
    # 搜索字段：msku，asin，sku
    sortColumn: str
    # 排序字段
...
class ServiceCustomerservicecrmcustomerindexRequest(RequestBase):
    ...
    # 筛选结束时间
    currency_type: Optional[float] = None
    # 币种，0=原币种，1=CNY，2=USD
    search_field: Optional[str] = None
    # 支持搜索的字段 buyer_email、buyer_name
...
class ServicePerformancenoticelistRequest(RequestBase):
    ...
    # 处理状态：0（无），1（待处理），2（已处理），3（无需处理）
    startDate: Optional[str] = None
    # 开始时间 YYYY-MM-DD
    endDate: Optional[str] = None
    # 结束时间 YYYY-MM-DD
    searchField: Optional[str] = None
    # 搜索字段,subject 邮件主题,content 邮件内容
    searchValue: Optional[str] = None
    # 搜索值
    mailTagIds: Optional[list] = None
    # 邮件标签 id
    isRead: Optional[float] = None
    # 是否已读，-1 全部，0 未读，1 已读
    offset: Optional[float] = None
    # 偏移量
    length: Optional[float] = None
