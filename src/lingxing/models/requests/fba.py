...
class FBAShipmentPlanListsRequest(LingXingModel):
    ...
    search_field_time: Optional[str] = (
        None  # 查找时间字段(gmt_create-创建时间,estimated_delivery_time-计划发货时间)，不传该字段默认为gmt_create
    )
...
class FBAGetInboundShipmentListRequest(LingXingModel):
    ...
    time_type: Optional[int] = (
        None  # 时间类型：  3创建时间 (允许精确到时分秒)  2创建时间  1到货时间   0发货时间  4更新时间 (允许精确到时分秒)
    )
...
class FBACreatereadysendorderRequestListItem(LingXingModel):
    ...
    cg_product_gross_weight: Optional[str] = (
        None  # 采购：单品毛重（G）；不传值则自动取【商品管理】模块维护的SKU“单品毛重”；按计费重分摊时需用到该字段
    )
    cg_product_net_weight: Optional[str] = (
        None  # 采购：单品净重（G）；不传值则自动取【商品管理】模块维护的SKU“单品净重”；
    )
    cg_package_length: Optional[str] = (
        None  # 采购：包装规格-长（CM）；不传值则自动取【商品管理】模块维护的SKU“包装规格-长”；按计费重分摊时需用到该字段
    )
    cg_package_width: Optional[str] = (
        None  # 采购：包装规格-宽（CM）；不传值则自动取【商品管理】模块维护的SKU“包装规格-宽”；按计费重分摊时需用到该字段
    )
    cg_package_height: Optional[str] = (
        None  # 采购：包装规格-高（CM）；不传值则自动取【商品管理】模块维护的SKU“包装规格-高”；按计费重分摊时需用到该字段
    )
    cg_box_weight: Optional[str] = (
        None  # 采购：单箱重量（KG）；不传值则自动取【商品管理】模块维护的SKU“单箱重量”；按计费重分摊时需用到该字段
    )
...
class FBACreatereadysendorderRequestHeadLogisticsListItem(LingXingModel):
    ...
    tracking_list__order_type_code: Optional[int] = (
        None  # 单号类型：【注意：与运输类型联动关系】 1 订舱号 2 提单号 3 箱号 4 其他 5 跟踪单号 6航班号 当transport_type=1时只能传5 当transport_type=2时只能传1、
    )
    tracking_list__shippers: Optional[str] = (
        None  # 承运商，运输类型为海运时才有意义： 获取发货单头程物流-承运商信息接口获取
    )
...
class FBACreatereadysendorderRequest(LingXingModel):
    ...
    head_fee_type: Optional[int] = (
        None  # 头程费分配方式：【默认0】 0 按计费重 1 按实重 2 按体积重 3 按SKU数量 4 自定义 5 按箱子体积
    )
    tax_fee_type: Optional[int] = (
        None  # 实际税费分配方式：【默认0】 0 产品-计费重 1 产品-实重 2 产品-体积重 3 产品-数量 4 自定义 5 箱子-体积
    )
    ...
    logistics_channel_id: Optional[int] = (
        None  # 物流渠道id：按计费重分摊时必填，以获取材积参数用于计算 查询头程物流渠道列表接口对应字段【id】
    )
...
class FBACreateSendedOrderRequestListItem(LingXingModel):
    ...
    tax_amount: Optional[str] = None  # 税费值
    cg_product_gross_weight: Optional[str] = (
        None  # 采购：单品净重（G）；不传值则自动取【商品管理】模块维护的SKU“单品毛重”；按计费重分摊时需用到该字段
    )
    cg_package_length: Optional[str] = (
        None  # 采购：包装规格-长（CM）；不传值则自动取【商品管理】模块维护的SKU“包装规格-长”；按计费重分摊时需用到该字段
    )
...
class FBACreateSendedOrderRequest(LingXingModel):
    ...
    tax_fee_type: Optional[int] = (
        None  # 实际税费分配方式：【默认0】 0 产品-计费重 1 产品-实重 2 产品-体积重 3 产品-数量 5 箱子-体积
    )
    is_points_behind: Optional[int] = None  # 是否分抛计算：0 否，1 是，头程分摊方式为按计费重时用
    points_behind_coeffient: Optional[int] = None  # 分抛系数：0~100，分抛计算选是时必填
    logistics_channel_id: Optional[int] = (
    ...
    request_flag: Optional[str] = (
        None  # 自定义请求标识，本次请求超时后可根据此标识查询此次请求的结果，由请求方保持标识唯一性
    )
    ship_mode: Optional[int] = None  # 发货方式：1-默认，2-工厂直发
    hand_pick_purchase: Optional[int] = None  # 工厂直发时手动选择出库批次：1-否，2-是
    remark: Optional[str] = None  # 备注
...
class FBAShipmentLockStockRequest(LingXingModel):
    ...
    is_auto_batch: Optional[int] = (
        None  # 是否锁定至批次，1：是，0：否，默认为否，否：只锁定库存数量，发货时按先进先出规则匹配出库批次；是：按先进先锁规则自动指定批次并锁定，发货时按锁定批次出库；分配库存后，可在【查询发货单详情】接口的采购
    )
...
class FBAUpdatelistlogisticsRequestDataItem(LingXingModel):
    ...
    actual_shipment_time: Optional[str] = None  # 实际发货时间，格式：Y-m-d
    tax_fee_type: Optional[int] = (
        None  # 实际税费分配方式：【默认0】 0 产品-计费重 1 产品-实重 2 产品-体积重 3 产品-数量 4 自定义 5 箱子-体积/体积重 6 箱子-计费重 7 箱子-实重 8 产品-根据清关单价*税率占比
    )
    logistics_channel_id: Optional[int] = (
        None  # 物流渠道id：按计费重分摊时必填，以获取材积参数用于计算 查询头程物流渠道列表接口对应字段【id】
    )
    logistics_list_type: Optional[int] = None  # 物流信息版本： 1 新版
    head_logistics_list: Optional[dict] = (
        None  # 新版头程物流信息 【对应 logistics_list_type = 1】 【注意：新版头程物流数据为覆盖式更新，包括tracking_list、estimate_expenses_list、actu
    )
    ...
    head_logistics_list__tracking_list__shippers: Optional[str] = (
        None  # 承运商，运输类型为海运时才有意义： 获取发货单头程物流-承运商信息接口获取
    )
    ...
    head_logistics_list__estimate_expenses_list__other_fee_arr: (
        List  # 预估费用-其他费： 获取发货单头程物流-其他费类型接口获取
    )
    ...
    head_logistics_list__actual_expenses_list__other_fee_arr: (
        List  # 实际费用-其他费： 获取发货单头程物流-其他费类型接口获取
    )
...
class FBACreateSTATaskRequestInboundplanitemsItem(LingXingModel):
    ...
    prepCategory: Optional[str] = (
        None  # 预处理分类：（当商品未在卖家中心设置过预处理指导时可填写）  ADULT-成人：对应的预处理类型通常为无需预处理[ITEM_NO_PREP]，也可传其他预处理类型  HANGER-悬挂在衣架上的服装：
    )
    prepTypes: Optional[list] = (
        None  # 预处理类型：（当商品未在卖家中心设置过预处理指导时可填写）  ITEM_POLYBAGGING-聚乙烯塑料袋包装：将商品放在透明袋中，以防破损、落尘或泄漏。包装袋必须印有窒息警告，必须密封（如果不是自
    )
...
class FBASyncShipmentRequest(LingXingModel):
    ...
    sync_anyway: Optional[int] = (
        None  # 报错是否继续：0 否【默认】，1 是 当系统检测到货件归属国家与店铺不符时，会提示报错，此时传1则按照店铺进行同步
    )
...
class FBAPrintfbalabelsRequest(LingXingModel):
    ...
    print_sta_name_page: Optional[int] = (
        None  # 传值1为新增任务名称页,默认不新增,非必填,仅打印box箱子标签时生效,传值1为开启
    )
    sort_label: Optional[int] = (
        None  # 传值1为按箱子顺序重排,默认不按箱子顺序重排,仅打印box箱子子标签时生效(说明:不按箱子顺序重排时,打印文件...
    )
...
class FBAFBAShipmentListRequest(LingXingModel):
    ...
    shipment_status: Optional[str] = (
        None  # 货件状态，多个以英文逗号分隔： UNCONFIRMED IN_TRANSIT DELIVERED CHECKED_IN ABANDONED  DELETED CLOSED CANCELLED WORK
    )
    extra_date_field: Optional[str] = (
        None  # 根据start_extra_date和end_extra_date日期范围查询： update 货件修改日期【默认值为update，目前只支持查询货件修改日期】
    )
...
class FBAGetFbaProductListRequest(LingXingModel):
    ...
    search_field: Optional[str] = (
        None  # 模糊搜索字段：【搜索时支持以下单个字段】 msku=>MSKU fnsku=>FNSKU asin=>ASIN sku=>SKU title=>标题 product_name=>品名
    )
