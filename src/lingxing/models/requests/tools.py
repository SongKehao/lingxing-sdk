...
class ToolsWarningmessagegoodslistRequest(LingXingModel):
    ...
    model_id_list: Optional[list] = (
        None  # 预警模型： 1  Listing调价预警 2  FBA费变更预警 3  Listing下架预警 6  FBA费异常预警 7  折扣异常预警 18  业务指标预警 20  折扣叠加预警 21  buyb
    )
...
class ToolsWarningmessageinventorylistRequest(LingXingModel):
    ...
    length: Optional[int] = None  # 分页长度，默认50，上限200
    model_id_list: Optional[list] = (
        None  # 预警模型：  4  本地库存预警 5  亚马逊库存预警 22  本地库龄预警 23  亚马逊库龄预警
    )
