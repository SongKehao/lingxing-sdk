...
class WarehouseAwdinboundplancreateplanRequestAwddeliveredgoodsbosItem(LingXingModel):
    ...
    msku: str  # msku
    prepCategory: Optional[str] = None  # 预处理类别：ADULT：成人；HANGER：悬挂在衣架上的服装；TEXTILE：服装、面料、毛绒玩具和纺织品；BABY：母婴用品；FRAGILE：易碎品；LIQUID：液体（未存放在玻璃容器中）；PE
...
class WarehouseAwdinboundplancreateplanRequest(LingXingModel):
    ...
    """
    destinationRegion: Optional[str] = None  # 地区偏好：us-east：美国东海岸（马里兰州和宾夕法尼亚分拨中心）；us-west：美国西海岸（加利福尼亚州分拨中心）；us-southcentral：美国中南部（德克萨斯州分拨中心）；us-sou
    sid: int  # 店铺id，对应查询亚马逊店铺列表接口对应字段【sid】
...
class WarehouseAwdinboundplanpageRequest(LingXingModel):
    ...
    statusList: Optional[list] = None  # 任务状态：LOCALDRAFT：草稿；DRAFT：待确认；VALIDATING：更新中；CONFIRMED：已确认；CLOSED： 已关闭；EXPIRED：已过期；CANCELLED：已取消
...
class WarehouseAwdinboundshipmentpageRequest(LingXingModel):
    ...
    length: int  # 分页大小，上限
    startDateTime: str  # 开始时间，格式：YYYY-MM-DD  双闭区间
    statusList: Optional[list] = None  # 任务状态：CREATED：已创建；SHIPPED：已发货；IN_TRANSIT：运输中；RECEIVING：接收中；DELIVERED：已送达；CLOSED：已关闭；CANCELLED：已取消
...
class WarehouseFBAStockV2Request(LingXingModel):
    ...
    query_fba_storage_quantity_list: Optional[bool] = None  # true 是、false 否；默认false，如果传入true,则出参数据中的欧洲共享仓会将出参字段-fba_storage_quantity_list的值返回
...
class WarehouseInventorybindetailsRequest(LingXingModel):
    ...
    bin_type_list: Optional[str] = None  # 仓位类型，多个类型用英文逗号分隔： 1 待检暂存 2 可用暂存 3 次品暂存 4 拣货暂存 5 可用 6 次品
    offset: Optional[int] = None  # 分页偏移量，默认0
...
class WarehouseWarehouseStatementRequest(LingXingModel):
    ...
    type: Optional[str] = None  # 流水类型：【多个流水类型用英文逗号分隔，不填默认全部类型】  1 其他入库 2 采购入库 3 调拨入库 10 其它入库（已撤销） 11 其他出库 12 FBA出库 13 调拨出库 14 退货出库 15
...
class WarehouseWarehousebinstatementRequest(LingXingModel):
    ...
    type: Optional[str] = None  # 流水类型：【多个流水类型用英文逗号分隔，不填默认全部类型】 16 换标入库 17 加工入库 18 拆分入库 19 其他入库 22 采购入库 23 委外入库 24 调拨入库 25 盘盈入库 26 退货入
    bin_type_list: Optional[str] = None  # 仓位类型：【多个类型用逗号分隔】 1 待检暂存 2 可用暂存 3 次品暂存 4 拣货暂存 5 可用 6 次品
...
class WarehouseReceiptOrderQcListRequest(LingXingModel):
    ...
    status: Optional[str] = None  # 状态，多个使用英文逗号分隔： 0 待质检 1 已质检 2 已免检 10 已质检（撤销） 20 已免检（撤销）
...
class WarehouseOrderAddRequest(LingXingModel):
    ...
    supplier_id: Optional[str] = None  # 自定义供应商id【supplier_id、sys_supplier_id 二选一必填，都填优先取supplier_id】
    sys_supplier_id: Optional[int] = None  # 系统供应商id【supplier_id、sys_supplier_id 二选一必填，都填优先取supplier_id】
...
class WarehouseInboundgetordersRequest(LingXingModel):
    ...
    search_field_time: Optional[str] = None  # 日期筛选类型： 创建时间 create_time 入库时间 opt_time 更新时间 increment_time
    ...
    type: Optional[int] = None  # 入库类型： -1 其他入库（含所有自定义类型）  1 其他入库（非自定义类型） 2 采购入库 3 调拨入库 4 赠品入库 26 退货入库 27 移除入库
...
class WarehouseOrderAddOutRequest(LingXingModel):
    ...
    sys_supplier_id: Optional[int] = None  # 系统客户供应商ID（退货出库：客户供应商ID, sys_supplier_id和supplier_id其中一个必填，都填则取supplier_id）
    supplier_id: Optional[str] = None  # 客户供应商ID（退货出库：客户供应商ID, sys_supplier_id和supplier_id其中一个必填，都填则取supplier_id）
...
class WarehouseOutboundgetordersRequest(LingXingModel):
    ...
    search_field_time: Optional[str] = None  # 日期筛选类型： 创建时间 create_time 出库时间 opt_time 更新时间 increment_time
    start_date: Optional[str] = None  # 日期查询开始时间，格式：Y-m-d 当筛选更新时间时，支持Y-m-d或Y-m-d H:i:s
    end_date: Optional[str] = None  # 日期查询结束时间，格式：Y-m-d 当筛选更新时间时，支持Y-m-d或Y-m-d H:i:s
    ...
    type: Optional[int] = None  # 出库类型： 11 其他出库 12 FBA出库 14 退货出库 15 调拨出库 16 WFS出库 17 Temu出库 18 销毁出库
...
class WarehouseAddstorageprocessorderRequestProductListItem(LingXingModel):
    ...
    combo_whb_code: Optional[str] = None  # 加工单组合品入库仓位，不传默认暂存可用；拆分单不用传 【查询本地仓位列表 接口对应字段【storage_bin】】
    ...
    single_product_list__whb_code: Optional[str] = None  # 拆分单的单品入库仓位，不传默认可用暂存；加工单不用传 【查询本地仓位列表 接口对应字段【storage_bin】】
...
class WarehouseGetprocessorderlistsRequest(LingXingModel):
    ...
    search_field_time: Optional[Any] = None  # 时间搜索维度： create_time 创建时间 finish_time 完成时间 update_time 更新时间
...
class WarehouseGetstorageallocationlistRequest(LingXingModel):
    ...
    search_date_type: Optional[int] = None  # 时间类型：【不传或传空则默认为 1】 1 创建时间 2 调拨时间 3 完成时间 4 更新时间
...
class WarehouseAddAllocationOrderRequest(LingXingModel):
    ...
    remark: Optional[str] = None  # 备注
    type: Optional[int] = None  # 调拨类型：【默认1】 1 简易调拨【创建已完成状态的单据】 2 完整调拨【创建待收货状态的单据】
...
class WarehouseAddrebrandadjustmentorderRequestProductListItem(LingXingModel):
    ...
    out_available_bin__num: Optional[int] = None  # 出库数量，默认按ERP页面逻辑出库 【out_available_bin>>num 之和 等于 adjustment_valid_num】
    in_available_bin: Optional[list] = None  # 入库仓位列表，默认可用暂存
    in_available_bin__whb_code: Optional[str] = None  # 入库仓位
    in_available_bin__num: Optional[int] = None  # 入库数量，当填写入库仓位时，该字段必填 【in_available_bin>>num 之和 等于 adjustment_valid_num】
...
class WarehouseCheckgetorderlistRequest(LingXingModel):
    ...
    check_type: Optional[str] = None  # 盘点类型，多个盘点类型用英文逗号分隔： 1 整仓盘点 2 SKU盘点 3 仓位盘点 4 SKU+仓位盘点
    ...
    search_value: Optional[str] = None  # 搜索值
    status: Optional[int] = None  # 盘点状态： 10 待盘点 20 预锁 30 盘点中 40 已盘点 121 待审核 122 已驳回 123 通过 124 作废
...
class WarehouseCheckgetorderdetailRequest(LingXingModel):
    ...
    """
    order_sn: str  # 盘点单号
    search_field: Optional[str] = None  # 搜索字段： sku SKU fnsku FNSKU product_name 品名 whb_code_text 仓位 whb_type_text 仓位类型
    search_value: Optional[str] = None  # 搜索值
    sort_field: Optional[str] = None  # 排序字段： book_inventory 账面库存 actual_inventory 实盘库存 different_count 库存差异
...
class WarehouseSubmitallocationorderRequest(LingXingModel):
    ...
    freight_fee: Optional[str] = None  # 运费
    other_fee: Optional[str] = None  # 其他费用
    fee_part_type: Optional[int] = None  # 费用分摊方式：0 不分摊【默认值】，2 按sku数量分摊，3 按重量，4 按体积，5 按自定义
    remark: Optional[str] = None  # 备注
...
class WarehouseListinboundRequest(LingXingModel):
    ...
    page: Optional[int] = None  # 当前页码，默认1
    date_type: Optional[str] = None  # 备货单时间查询类型：【默认create_time】 delivery_time 发货时间 create_time 创建时间 receive_time 收货时间 update_time 更新时间
...
class WarehouseUpdateinboundRequestHeadLogisticsListItem(LingXingModel):
    tax_fee_type: int  # 税费分摊方式：  0：产品-计费重  1：产品-实重  2：产品-体积重  3：产品-数量  4：自定义  5：箱子-体积)
...
class WarehousePackingRequestBoxListItem(LingXingModel):
    ...
    items__twp_id: Optional[int] = None  # 三方商品id，查询系统产品与第三方海外仓产品映射列表接口对应字段【twp_id】， 注：收货仓库是第三方仓时必填
...
class WarehouseCreateInboundRequestProductListItem(LingXingModel):
    ...
    oversea_product_code: Optional[str] = None  # 三方sku编码，查询系统产品与第三方海外仓产品映射列表接口对应字段【oversea_product_code】 【当仓库是有api海外仓必填】
...
class WarehouseCreateInboundRequest(LingXingModel):
    ...
    status: Optional[int] = None  # 订单状态：【默认60】 40 待发货 50 待收货 60 已完成 注：收货仓支持三方海外仓的备货单状态只会到待发货
    estimated_time: Optional[str] = None  # 预计到货时间
    arrival_time: Optional[str] = None  # 实际到货时间
    share_id: Optional[int] = None  # 头程费分摊方式：【默认0】 0 按计费重 1 按实重 2 按体积重 3 按SKU数量 4 自定义 5 按箱子体积 注意：生成待发货状态备货单时，需要通过接口上传备货单装箱信息上传箱子信息； 待收货和已
    remark: Optional[str] = None  # 备注
    file_id: Optional[str] = None  # 附件id
    overseas_type: Optional[int] = None  # 下单至第三方【默认2】： 1 否，2 是 注：当收货仓为API海外仓时可填，不填默认为是
...
class WarehouseRemovalinboundlistRequest(LingXingModel):
    ...
    """
    status: Optional[int] = None  # 订单状态： 1 待提交-未提交 2 待提交-提交中 3 待提交-失败 4 待收货-未收货 5 待收货-异常 6 已完成 7 已作废
...
class WarehouseWmsOrderListRequest(LingXingModel):
    ...
    logistics_status_arr: Optional[list] = None  # 物流状态： 1 待导入 2 物流待下单 3 物流下单中 4 下单异常 5 下单完成 6 待海外仓下单 7 海外仓下单中 11 待导入国内物流 41 物流取消中 42 物流取消异常 43 物流取消完成
    ...
    time_type: Optional[str] = None  # 时间类型： 创建时间 create_at  出库时间【单据操作】 delivered_at 流水出库时间 stock_delivered_at 变更时间 update_at
...
class WarehouseSettrackingnoRequest(LingXingModel):
    ...
    logistics_freight_currency_code: Optional[str] = None  # 物流运费币种： CNY USD EUR JPY AUD CAD MXN GBP INR AED SGD SAR BRL SEK PLN TRY HKD
