...
class StatisticsAsinListNewRequestExtendSearchItem(LingXingModel):
    field: Optional[str] = None  # 筛选字段，对应返回字段的字段名，含义参考返回字段，只支持以下字段： volume order_items amount volume_chain_ratio order_chain_ratio amo
    from_value: Optional[int] = None  # 数值： 1、当exp为gt、lt、ge、le、eq 时，比较值填充至此字段； 2、当exp为range时，左区间值填充至此字段；
...
class StatisticsAsinListNewRequest(LingXingModel):
    ...
    search_field: Optional[str] = None  # 搜索字段，目前支持字段如下： asin parent_asin msku local_sku【sku】 item_name【标题】
    ...
    purchase_status: Optional[int] = None  # 退货退款统计方式： 0 默认值，表示按退货退款的发生时间统计退货退款数据 1 表示按下单时间统计退货退款数据
...
class StatisticsProfitMskuRequest(LingXingModel):
    ...
    currency_type: Optional[str] = None  # 币种，默认原币种 1 人民币-CNY 2 美元-USD 3 欧元-EUR 4 日元-JPY 5 澳元-AUD 6 加拿大元-CAD 7 墨西哥比索-MXN 8 英镑-GBP 9 印度卢比-INR 10
    sort_field: Optional[str] = None  # 排序字段：asin
    sort_type: Optional[str] = None  # desc:倒序   asc:顺序
...
class StatisticsReimbursementListRequest(LingXingModel):
    ...
    search_field: Optional[str] = None  # 搜索字段： reimbursement_id 赔偿编号 amazon_order_id 订单号 asin ASIN msku MSKU fnsku FNSKU item_name 标题
...
class StatisticsPurchaseReportProductListRequest(LingXingModel):
    ...
    search_field: Optional[str] = None  # 搜索字段名： product_name 品名 sku SKU msku MSKU fnsku FNSKU spu_name 款名 spu SPU
    search_value: Optional[str] = None  # 搜索值
...
class StatisticsReturnOrderAnalysisListsRequest(LingXingModel):
    ...
    asinType: Optional[str] = None  # 维度类型，枚举值：msku, asin, parentAsin, sku, spu（注意：不支持sid、country、category、band）
    ...
    searchField: Optional[str] = None  # 搜索字段类型，枚举值：msku-MSKU, asin-ASIN, parentAsin-父ASIN, localSku-SKU, localName-品名, spu-SPU, spuName-款名
    searchValue: Optional[list] = None  # 搜索值列表，与searchField配合使用
    sortField: Optional[str] = None  # 排序字段，枚举值：curReturnGoodsCount-退货量, returnGoodsCountRatio-退货量环比, curVolume-销量, curReturnGoodsVolumeRat
...
class StatisticsPlatformStatisticsSaleStatPageListV2Request(LingXingModel):
    ...
    sids: Optional[list] = None  # 店铺id，多个使用英文逗号分隔。 如果id属于亚马逊店铺id，则对应查询亚马逊店铺列表接口对应字段【sid】  如果id属于多平台店铺id，则对应查询多平台店铺信息接口对应字段【store_id】
