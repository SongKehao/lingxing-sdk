...
Args:
    ...
    length: 分页长度，默认500，上限500, int."""
        resp = await self._post("/erp/sc/routing/data/local_inventory/purchaseOrderList", {k: v for k, v in {"start_date": start_date, "end_date": end_date, "search_field_time": search_field_time, "order_sn": order_sn, "custom_order_sn": custom_order_sn, "purchase_type": purchase_type, "offset": offset, "length": length}.items() if v is not None})
        return self._parse_list(resp.data, PurchaseOrderListItem)
    async def purchase_plan_cancel(self, plan_sn: Any = None, reason: str = None) -> dict:
        """作废采购计划.
