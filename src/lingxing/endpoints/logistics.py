...
class LogisticsEndpoints(BaseEndpoint):
    ...
    async def used_logistics_type(
        self, data: UsedLogisticsTypeReq | None = None
    ) -> list[WmsLogisticsListUsedLogisticsTypeResponse]:
        """查询已启用的自发货物流方式.
    ...
    async def head_reconciliation_list(
        self,
        offset: int = None,
        length: int = None,
        ...
        shipping_method_ids: list = None,
    ) -> list[LogisticsHeadReconciliationListResponse]:
        """头程对账列表.
        ...
        resp = await self._post(
            "/basicOpen/logistics/headLogisticsReconciliation/list",
            {
                k: v
                for k, v in {
                    "offset": offset,
    ...
    async def logistics_bill_confirm(
        self, data: LogisticsBillConfirmReq | None = None
    ) -> list[LogisticsBillConfirmResponse]:
        """FBM物流对账-确认/批量确认.
    ...
    async def add_channels(self, channelsData: list = None) -> TmsFirstvesselAddchannelsResponse | None:
        ...
        resp = await self._post(
            "/erp/sc/routing/tms/FirstVessel/addChannels",
            {k: v for k, v in {"channelsData": channelsData}.items() if v is not None},
        )
    ...
    async def add_providers(self, providersData: list = None) -> TmsFirstvesselAddprovidersResponse | None:
        ...
        resp = await self._post(
            "/erp/sc/routing/tms/FirstVessel/addProviders",
            {k: v for k, v in {"providersData": providersData}.items() if v is not None},
        )
    ...
    async def channel_list(self, offset: int = None, length: int = None) -> list[LocalInventoryChannellistResponse]:
        ...
        resp = await self._post(
            "/erp/sc/data/local_inventory/channelList",
    ...
    async def query_head_logistics_provider(self, search: dict = None) -> list[HeadlogisticsproviderQueryListResponse]:
        """查询物流-头程物流商.
        ...
        resp = await self._post(
            "/basicOpen/logistics/headLogisticsProvider/query/list",
            {k: v for k, v in {"search": search}.items() if v is not None},
        )
        return self._parse_list(resp.data, HeadlogisticsproviderQueryListResponse)

    async def transport_method_list(self, **kwargs: Any) -> list[BusinessconfigTransportmethodListResponse]:
        """查询运输方式列表.
