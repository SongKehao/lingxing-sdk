...
class TargetManageEndpoints(BaseEndpoint):
    ...
    async def store_batch_select(self, assess_year: int = None) -> list[StoreBatchselectResponse]:
        ...
        resp = await self._post(
            "/bd/goal/management/open/store/batchSelect",
            {k: v for k, v in {"assessYear": assess_year}.items() if v is not None},
        )
        return self._parse_list(resp.data, StoreBatchselectResponse)

    async def store_batch_operate(
        self, assess_year: int = None, currency_code: str = None, operate_type: int = None, goal_list: list = None
    ) -> list[StoreBatchoperateResponse]:
        """店铺维度-批量新增/更新目标.
        ...
        resp = await self._post(
            "/bd/goal/management/open/store/batchOperate",
            {
                k: v
                for k, v in {
    ...
    async def store_batch_delete(self, assess_year: int = None, sids: list = None) -> list[StoreBatchdeleteResponse]:
        ...
        Args:
            ...
            sids: 店铺id列表, array."""
        resp = await self._post(
            "/bd/goal/management/open/store/batchDelete",
            {k: v for k, v in {"assessYear": assess_year, "sids": sids}.items() if v is not None},
        )
        return self._parse_list(resp.data, StoreBatchdeleteResponse)

    async def user_batch_select(
        self, assess_year: int = None, assess_type: int = None
    ) -> list[UserBatchselectResponse]:
        """组织维度-批量查询目标.
        ...
        resp = await self._post(
            "/bd/goal/management/open/user/batchSelect",
            {k: v for k, v in {"assessYear": assess_year, "assessType": assess_type}.items() if v is not None},
        )
        return self._parse_list(resp.data, UserBatchselectResponse)

    async def user_batch_operate(
        self,
        assess_year: int = None,
        assess_type: int = None,
        currency_code: str = None,
        operate_type: int = None,
        user_goal_list: list = None,
    ) -> list[UserBatchoperateResponse]:
        """组织维度-批量新增/更新目标.
        ...
        resp = await self._post(
            "/bd/goal/management/open/user/batchOperate",
            {
                k: v
                for k, v in {
                    "assessYear": assess_year,
    ...
    async def user_batch_delete(
        self, assess_year: int = None, assess_type: int = None, uid_list: list = None
    ) -> list[UserBatchdeleteResponse]:
        """组织维度-批量删除目标.
        ...
        resp = await self._post(
            "/bd/goal/management/open/user/batchDelete",
            {
                k: v
                for k, v in {"assessYear": assess_year, "assessType": assess_type, "uidList": uid_list}.items()
                if v is not None
            },
        )
